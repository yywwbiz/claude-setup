...
Add a persona to `.claude/team.json`. The argument is optional.
...
2. Read `.claude/team.json` (create with `{"active": []}` if missing).
...
2. Read the current active roster from `.claude/team.json`.
...
5. After they select, add the chosen persona to `.claude/team.json` and confirm.
...
- The file lives at `<project>/.claude/team.json`, not the plugin directory.
